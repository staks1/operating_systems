...
{
	int i;
	for (i=0; i<level; i++)        //not important -->used for change of line
		printf("\t");
...
parse_node(FILE *file, struct tree_node *node)
{
	...
	/* here sos it replaces the node->nr_children with nr_children*/
	...
	/* read children names */  /* here maybe use alter */
